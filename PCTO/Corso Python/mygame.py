...
pygame.display.set_caption('Shooter Game Redux (Matteo Porcu)') # imposta il titolo della finestra
...
ship_surf=pygame.image.load("../sprites/ship.png").convert_alpha() # carica l'immagine del quadrato
...
while True: #faccio un ciclo infinito per mantenere la finestra aperta
    ...
    for laser_rect in laser_rects: # ciclo per disegnare i laser
        display_surface .blit (laser_surf, laser_rect) # disegna il laser
        laser_rect.move_ip(0,-500*dt) # muove il laser verso l'alto
        if laser_rect.bottom < 0: # se il laser è fuori dalla finestra
            laser_rects.remove(laser_rect) # elimina il laser dalla lista
